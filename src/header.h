...
void process_Action(string &entry, player &player, const vector<action> &actions, const vector<location> &locations);     //processes the player's command and updates the game state.
bool verify_EndConditions(const player &player);         //checks if the player has lost or won.
void discover_memory(player&player, string & memory);       //mandles the event of finding a new memory fragment.
void interact_with_Ara(player &player);      //manage dialogue and key decisions with the character ARA.
void handle_final_choice(player &player);           //manages the sequence of end-game decisions.
